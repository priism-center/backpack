...
```{r summary_standard, exercise=TRUE}
...
Run a principal component analysis using `princomp` on `seeds_standard` and assign it to the name `seeds_PCA`:  

```{r princomp, exercise=TRUE}

```

```{r princomp-solution}
seeds_PCA <- princomp(seeds_standard)
```
